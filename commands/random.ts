...
export default new ReadableCommand(
    ...
    async (interaction: ChatInputCommandInteraction) => {
        ...
        const count = (await randomBeep.reactions.cache.get(utils.emojis.hand)?.users.fetch({limit: 200})).filter(user => user.id !== randomBeepAuthor.id && user.id !== config.clientId).size
